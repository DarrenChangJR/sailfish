...
def get_solc_path(contract_path, log=None):
    ...
    solc_version_no_v = str(maximum_min_version.get_typed_version(VersionType.STRIPPED_VERSION))
    if log:
        log.info('solc version: %s' % solc_version)

    # Get solc binary path that can compile this contract
    solc_binary = 'solc-' + solc_version_no_v
    ...
    # Install otherwise
    print('solc_version_no_v: ', solc_version_no_v)
    print(AVAILABLE_SOLC_VERSION_PATHS)
